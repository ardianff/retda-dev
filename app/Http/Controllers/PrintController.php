   ...
   private function usulanPDF($pengajuan_id, $opd_id, $gol_id, $uppd_id)
{
   ...
   if ($uppd_id == 0) {
       # code...
     $uppd = Uppd::where('opd_id', $opd_id)
    ->orderBy('level', 'ASC')   // 🔥 level dulu (1 → 2 → 3)
    ->orderBy('id_grms', 'ASC')      // 🔥 baru id
    ->get();
   }else{
       $uppd = Uppd::where('id',$uppd_id)->get();
   ...
   private function unduhUsulanPDF($pengajuan_id, $opd_id, $gol_id, $uppd_id)
{
   ...
   if ($uppd_id == 0) {
       ...
       $uppd = Uppd::where('opd_id',$opd_id)->orderBy('level', 'ASC')
    ->orderBy('id_grms', 'ASC')->get();
...
$tu=TahunUsulan::find($pengajuan_id);
    ...
    if ($uppd_id == 0) {
        ...
        $uppd=Uppd::find($uppd_id);

    }

    $keseluruhan=$jumlah_kenaikan+$jumlah_penurunan+$jumlah_usulan_baru+$jumlah_perubahan_format+$jumlah_tetap;
     $pdf = PDF::loadView('print.usulan.rekap', compact('jumlah_kenaikan','jumlah_penurunan','jumlah_usulan_baru','jumlah_perubahan_format','jumlah_tetap','nonAktif','opd','uppd','tu','keseluruhan','uppd_id','opd_id'));
    ...
    private function draftPDF($pengajuan_id, $opd_id, $gol_id, $uppd_id)
{
    ...
    $golongan=Golongan::find($gol_id);
    if ($uppd_id == 0) {
        $uppd = Uppd::where('opd_id',$opd_id)->where('status','Aktif')->orderBy('level', 'ASC')
    ->orderBy('id_grms', 'ASC')->get();
        $uppd_name='Keselururhan';
    }else{
        $uppd = Uppd::where('id',$uppd_id)->where('status','Aktif')->orderBy('kode','ASC')->get();
        $uptd=Uppd::find($uppd_id);
        $uppd_name=$uptd->nama;
    }
   $data = Jenis::where('golongan_id', $gol_id)
...
private function draftExcel($pengajuan_id, $opd_id, $gol_id,$uppd_id)
{
   ...
   ->orderBy('id','ASC')->get();
    ...
    return view('print.lampiran.pdf', compact('data','uppd','golongan','opd','ta','pejabat'));
        
    }


    private function lampiranExcel($ta_id, $opd_id, $gol_id,$uppd_id)
    {
        $ta=TA::find($ta_id);
        $opd=Opd::find($opd_id);
        $golongan=Golongan::find($gol_id);
...
->with(['tarif' => function ($query) use ($ta_id) {
   ...
   ->orderBy('id','ASC')->get();    
        ...
        return Excel::download(new LampiranExcel( $golongan, $opd,$uppd,$data,$ta), 'Lampiran_Tarif_'.$opd->opd.'.xlsx');
    }
